...
describe("mondayIndex", () => {
  // mondayIndex is used throughout the dashboard alongside local-time
  // helpers (daysAgoStart/startOfLocalDay), so it deliberately reads
  // getDay() in local time. Build these dates with the local-components
  // constructor (year, month, day) rather than a "YYYY-MM-DD" string,
  // which Date parses as UTC midnight and would shift a day backwards
  // in any timezone west of UTC — a test bug, not a mondayIndex bug.
